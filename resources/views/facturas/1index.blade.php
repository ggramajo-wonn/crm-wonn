      ...
      <tbody>
        @forelse ($items as $i)
          @php
            $pagado = $i->paidSum();
            $saldo  = $i->balance();
            $cred   = method_exists($i, 'creditApplied') ? $i->creditApplied() : 0;
          @endphp
          <tr class="border-t border-gray-800">
            <td class="p-3 whitespace-nowrap">{{ $i->client->name }}</td>
            <td class="p-3">{{ optional($i->issued_at)->format('d/m/Y') }}</td>
            <td class="p-3">{{ optional($i->due_at)->format('d/m/Y') }}</td>
            <td class="p-3">{{ number_format($i->total, 2, ',', '.') }} $</td>
            <td class="p-3">{{ number_format($pagado, 2, ',', '.') }} $</td>            <td class="p-3">{{ number_format($saldo, 2, ',', '.') }} $</td>
            ...
            <td class="p-3 text-right space-x-2">
              ...
              <a href="{{ route('pagos.create', ['client_id'=>$i->client_id]) }}" class="text-emerald-300 hover:underline">Registrar pago</a>
