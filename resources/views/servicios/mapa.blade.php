...
@section('content')
@php
  // Intentamos obtener la API key de Google Maps desde empresa o .env
...
<script>
  // ====== Dataset recibido desde el controlador ======
  ...
  function initMap(){
    ...
    const napMarkers = NAPS.map(n => {
      ...
      m.addListener('click', () => { iw.setContent(cardNap(n)); iw.open({anchor:m, map}); });
      return m;
    });
